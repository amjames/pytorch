                ...
                if symbol_name in cur_kernel.fn.__globals__:
                    ...
                    if isinstance(symbol, JITFunction):
                        ...
                        if annotation := global_annotations.get(symbol_name):
                            annotation_code = _normalize_annotation(annotation)
                            compile_wrapper.writeline(f"{symbol_name}: {annotation_code} = {symbol!r}")
                        else:
                            compile_wrapper.writeline(f"{symbol_name} = {symbol!r}")
                        symbols_included.add(symbol_name)
                    elif isinstance(symbol, constexpr):
                        compile_wrapper.newline()
                        compile_wrapper.writeline(f"{symbol_name} = tl.constexpr({symbol.value!r}")
                        symbols_included.add(symbol_name)
                    elif (
